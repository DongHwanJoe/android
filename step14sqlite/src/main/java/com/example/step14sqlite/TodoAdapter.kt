...
/*
    Context type, Int type, List<Todo> type을 인자로 전달받는 대표 생성자
    인자로 전달받은 내용은 자동으로 필드에 저장된다.
...
class TodoAdapter(var context:Context, var layoutRes:Int, var list:List<Todo>) : BaseAdapter() {
    ...
    //인자로 전달된 position에 해당하는 아이템 리턴
    override fun getItem(position: Int): Any {
        return list.get(position)
    }

    //인자로 전달된 position에 해당하는 아이템의 아이디 리턴
    override fun getItemId(position: Int): Long {
        return list.get(position).num.toLong() //Int type을 Long type으로 casting 해서 리턴턴
    }
